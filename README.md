...
- 眾多杜著述的顯示：如何於某杜詩下，幷列數個、乃至數十個杜著述（如仇兆鰲的《杜詩詳註》、浦起龍《讀杜心解》、楊倫《杜詩鏡銓》、蕭滌非主編《杜甫全集校注》、謝思煒《杜甫集校注》）之注釋、評論、校記？
	...
	- 幷列的杜著述：
		...
		- 蕭滌非主編《杜甫全集校注》
		- 陳永明《杜甫全集粵音注音》
- 一對多：如何以同一組資料，生成數以千計的、內容各自不同的幷列杜著述、幷列版本、詩文與注釋、評論的部分分列的文檔（如只有《北征》的頭十句及其注釋，或只有《北征》的詩文以及各家對《北征》的評論）？
- 眾多全集：如何生成杜甫全集帶各詩的白話文翻譯（或英譯、日譯）、杜甫全集帶王嗣奭《杜臆》的評論、杜甫全集帶平仄、韻部說明，或者一部極具規模的杜甫全集匯注、匯校、匯評？
- 局部視窗（zooming in using a viewport）：如何以一視窗，只顯示如《北征》中關於「不聞夏殷衰，中自誅襃妲」一行的各家注釋或校記？
...
- 模型（Model）、面貌（View）： `CanonicalTextTrees`，杜著述正文樹、JSON/JSONL、後設資料指令樹、以控制器生成的面貌文檔樣本、杜著述引用書籍（《論語》、《史記》等）正文樹
...
- 基準正文樹：儲存杜甫詩文基準正文
- 杜著述正文樹、引用古籍正文樹：儲存基準正文以外的文字內容
- 後設資料指令樹：聯係基準正文樹、杜著述正文樹、引用古籍正文樹的指令集
...
- 以樹爲基礎，可生成路徑→文字（以路徑提取文字）、文字→路徑（以文字之定位）之各種索引
...
- **杜著述**指出現於是杜甫之後，討論杜甫或他的作品的任何文獻
- **引用古籍**指在某杜著述中被引用的、成書於杜甫之前或與之同時的典籍，像《詩經》、《論語》、《史記》等
...
**後設資料**指獨立於杜甫詩文、杜著述、引用典籍之外的、描述原始資料的第二級、甚至第三級的資料。第二級之後設資料負責聯係基準正文樹、杜著述正文樹/引用古籍正文樹。第三級後設資料（後設-後設資料）可修改儲存第二級資料之後設資料指令樹，幷提供爲第二級後設資料之分類、統計而設立之機制。

**後設資料指令樹**爲具層級結構的指令集，每條樹路徑均爲一特定指令，負責聯係前述之三種正文樹。
...
- 後設資料指令樹除提供指令路徑外，亦提供後設-後設資料

---

## 正文樹之坐標、路徑

<p>貫穿整個資料庫之定位鍵乃一坐標系統。例：</p>
<ul>
...
有別於正文樹，後設資料指令樹幷不儲存文字資料。後設資料指令樹乃一儲存指令之樹型結構，從樹根開始，到某個終點節點爲止，其間的樹路徑乃一指令。例子：
...
樹結構（tree structure) 有一個特點：樹中任何節點（node），連帶整棵以此節點爲根（root）之子樹，可以從樹中抽離，成爲獨立的樹。在後設資料指令樹中，注釋、評論等節點（詩碼之子節點）均可以獨立出來，成爲獨立的樹。這意味著用者可以只用一個或多個子節點，而非用全樹。如此，用者可以生成一個包含像仇兆鰲《杜詩詳註》之注釋、楊倫《杜詩鏡銓》之評論、蕭滌非《杜甫全集校注》之備考之混合版本之面貌。自由組合，乃是此系統之一大特色。

---

## 資料樹

在後設資料指令執行以後，基準正文樹成爲一棵資料樹：
...
- 原始面貌：不加任何格式、裝飾，保留原有樹結構的樹、子樹的原有內容
- 呈現面貌：去除樹結構後，再加上格式、裝飾的文字內容

基準正文樹、評注原始資料+後設資料標記完全分離，可生成相應的資料樹。在此資料樹中，可用路徑提取任何子樹，並以原始面貌的形式展示子樹的內容。

資料樹亦可支持同一版本（如仇兆鰲之《杜詩詳註》）之五個不同呈現面貌：
